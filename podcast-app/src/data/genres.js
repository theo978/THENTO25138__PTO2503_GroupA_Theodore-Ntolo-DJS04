// Genre metadata for the podcast app.
// The podcast preview API only exposes genres by numeric id, so this file
// supplies the human-readable titles (and the canonical show -> genre
// mapping) used throughout the UI.
...
/**
 * Lookup map of genre id -> genre title, built once at module load.
 * @type {Map<number, string>}
 */
export const genreTitleById = new Map(genres.map((g) => [g.id, g.title]));

/**
 * Resolve an array of genre ids (as returned by the podcast preview API)
 * into their human-readable titles.
 *
 * @param {Array<number|string>} genreIds - Genre ids attached to a show.
 * @returns {string[]} Matching genre titles, in the same order. Unknown
 *   ids are silently skipped so a single bad id never breaks rendering.
 */
export function getGenreTitles(genreIds = []) {
  return genreIds
    .map((id) => genreTitleById.get(Number(id)))
    .filter(Boolean);
}
